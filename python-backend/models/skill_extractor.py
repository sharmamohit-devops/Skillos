...
class SkillExtractor:
    ...
    def extract_structured_skills(self, text: str) -> Dict[str, List[str]]:
        ...
        for pattern in skill_patterns:
            ...
            for match in matches:
                skills = [
                    s.strip() for s in re.split(r"[,;|]", match) if len(s.strip()) > 2
                ]
                structured_skills["technical_skills"].extend(
                    skills[:5]
        ...
        for category in structured_skills:
            ...
            structured_skills[category] = [
                s for s in structured_skills[category] if len(s) > 2
            ]
    ...
    def detect_skill_depth(self, resume_text: str, skills: List[str]) -> Dict[str, str]:
        ...
        for skill in skills:
            ...
            advanced_count = sum(
                1
                for indicator in self.depth_indicators["advanced"]
            ...
            intermediate_count = sum(
                1
                for indicator in self.depth_indicators["intermediate"]
                if indicator in context_text
            )
            ...
            beginner_count = sum(
                1
                for indicator in self.depth_indicators["beginner"]
                if indicator in context_text
            )
    ...
    def calculate_role_based_weights(
        self, job_text: str, required_skills: List[str]
    ) -> List[Dict[str, str]]:
        ...
        for skill in required_skills:
            ...
            high_importance_found = any(
                re.search(pattern, job_text) for pattern in importance_patterns
            )
            ...
            skill_weights.append({"skill": skill, "importance": importance})
    ...
    def _detect_job_domain(self, job_text: str) -> str:
        ...
        return (
            max(domain_scores, key=domain_scores.get) if domain_scores else "fullstack"
        )

    def calculate_weighted_match_score(
        self, matched_skills: List[str], skill_weights: List[Dict[str, str]]
    ) -> float:
    ...
    def detect_risk_factors(
        ...
        if len(projects) < 2:
            risk_factors.append(
                "Limited project experience - only theoretical knowledge evident"
            )
        ...
        if not has_implementation:
            risk_factors.append(
                "No clear implementation experience - mostly theoretical background"
            )
        ...
        domain_match = any(
            domain.lower() in job_domain.lower() for domain in candidate_domains
        )
        if not domain_match:
            risk_factors.append(
                "Weak domain alignment - candidate background doesn't match job domain"
            )
        ...
        if len(critical_missing) > 3:
            risk_factors.append(
                f"Missing {len(critical_missing)} critical technical skills"
            )
        ...
        if skill_match_pct < 30:
            risk_factors.append(
                "Very low skill match - significant reskilling required"
            )

        # Risk 6: No recent experience
        if (
            "2023" not in resume_text
            and "2024" not in resume_text
            and "2025" not in resume_text
        ):
            risk_factors.append(
                "No recent experience mentioned - skills may be outdated"
            )
    ...
    def analyze_skills_advanced(self, resume_text: str, jd_text: str) -> Dict[str, Any]:
            ...
            if not best_match:
                for candidate_skill in candidate_skills:
                    similarity = self.calculate_skill_similarity(
                        required_skill, candidate_skill
                    )
        ...
        weighted_match_score = self.calculate_weighted_match_score(
            matched_skills, skill_weights
        )
        ...
        risk_factors = self.detect_risk_factors(
            resume_text, candidate_profile, job_analysis, skill_analysis
        )
    ...
    def analyze_skills(
        ...
        for req_skill in required_skills_norm:
            ...
            for cand_skill in candidate_skills_norm:
                ...
                if req_skill in self.skill_synonyms:
                    if cand_skill in [
                        s.lower() for s in self.skill_synonyms[req_skill]
                    ]:
                        matched_skills.append(req_skill)
                        best_match = cand_skill
                        break
            ...
            if best_match is None:
                ...
                best_match not in [s.lower() for s in matched_skills]
                and best_similarity > 0.7
            ):
        ...
        skill_match_percentage = (
            (matched_count / total_required * 100) if total_required > 0 else 0
        )
    ...
    def _analyze_skill_gap(
        self, skill: str, gap_type: str, job_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _determine_importance(self, skill: str, job_analysis: Dict[str, Any]) -> str:
        ...
        if skill_lower in [
            s.lower() for s in self.skill_taxonomy["programming_languages"]
        ]:
            return "High"
    ...
    def _determine_expected_depth(
        self, skill: str, job_analysis: Dict[str, Any]
    ) -> str:
    ...
    def evaluate_candidate(
        ...
        experience_score = self._calculate_experience_score(
            candidate_profile, job_analysis
        )
    ...
    def _calculate_experience_score(
        self, candidate_profile: Dict[str, Any], job_analysis: Dict[str, Any]
    ) -> float:
        ...
        if "senior" in required_exp and "senior" in candidate_exp:
            return 90.0
        elif "mid" in required_exp and (
            "mid" in candidate_exp or "senior" in candidate_exp
        ):
            return 85.0
    ...
    def _identify_strengths(
        self, candidate_profile: Dict[str, Any], skill_analysis: Dict[str, Any]
    ) -> List[str]:
