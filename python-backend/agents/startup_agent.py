...
class StartupAgent(BaseAgent):
    ...
    def _calculate_breadth(self, skill_analysis: Dict[str, Any]) -> int:
        ...
        has_frontend = any(
            skill in str(matched_skills).lower()
    ...
    def _calculate_startup_fit(
        ...
        matched_skills = skill_analysis.get("matched_skills", [])
        startup_tool_count = sum(
            1 for tool in self.startup_tools if tool in str(matched_skills).lower()
        )
    ...
    def _identify_excitement(
        ...
        if len(matched_skills) > 8:
            excited.append(
                f"Broad skill set - {len(matched_skills)} relevant technologies"
            )

        # Modern stack
        modern_tech = ["react", "vue", "docker", "kubernetes", "aws", "typescript"]
        modern_count = sum(
            1 for tech in modern_tech if tech in str(matched_skills).lower()
        )
        if modern_count >= 3:
    ...
    def _identify_concerns(
        ...
        if not any(
            verb in text_lower for verb in ["deployed", "shipped", "launched", "live"]
        ):
        ...
        # Large company only
        if any(
            word in text_lower for word in ["enterprise", "fortune 500", "corporate"]
        ):
            if "startup" not in text_lower:
                concerns.append(
                    "Only large company experience - startup pace may be challenging"
                )
    ...
    def _determine_verdict(
        self, execution: int, breadth: int, startup_fit: int, concerns: List[str]
    ) -> str:
    ...
    def _generate_comment(
        self, verdict: str, excited: List[str], concerns: List[str]
    ) -> str:
