...
class ATSAgent(BaseAgent):
    ...
    def evaluate(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        confidence = self._calculate_confidence(
            {"keyword": keyword_score, "format": format_score}
        )
    ...
    def _get_missing_keywords(self, skill_analysis: Dict[str, Any]) -> List[str]:
        ...
        if "skill_weights" in skill_analysis:
            ...
            if len(missing) < 5:
                medium_importance = self._extract_skills_by_importance(
                    skill_analysis, "Medium"
                )
                missing.extend(
                    [skill for skill in missing_skills if skill in medium_importance]
                )
        else:
    ...
    def _get_rejection_reasons(
        self, keyword_score: int, format_score: int, missing: List[str]
    ) -> List[str]:
    ...
    def _get_pass_reasons(
        self, keyword_score: int, format_score: int, skill_analysis: Dict[str, Any]
    ) -> List[str]:
    ...
    def _generate_comment(
        self, verdict: str, keyword_score: int, format_score: int
    ) -> str:
