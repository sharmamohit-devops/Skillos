...
class TechAgent(BaseAgent):
    ...
    def evaluate(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        confidence = self._calculate_confidence(
            {"depth": depth_score, "stack": stack_score, "complexity": complexity_score}
        )
        ...
        return {
            ...
            "concerns": concerns,
            "technical_comment": self._generate_comment(
                verdict, impressed_by, concerns
            ),
        }
    ...
    def _calculate_depth(self, resume_text: str, skill_analysis: Dict[str, Any]) -> int:
        ...
        if "skill_depth" in skill_analysis:
            ...
            advanced_count = sum(
                1 for depth in skill_depth.values() if depth == "advanced"
            )
            intermediate_count = sum(
                1 for depth in skill_depth.values() if depth == "intermediate"
            )
        ...
        advanced_count = sum(
            1 for concept in self.advanced_concepts if concept in text_lower
        )
    ...
    def _calculate_stack_coherence(self, skill_analysis: Dict[str, Any]) -> int:
        ...
        has_frontend = any(
            tech in str(matched_skills) for tech in modern_tech["frontend"]
        )
        has_backend = any(
            tech in str(matched_skills) for tech in modern_tech["backend"]
        )
        has_database = any(
            tech in str(matched_skills) for tech in modern_tech["database"]
        )
    ...
    def _calculate_complexity(self, resume_text: str, profile: Dict[str, Any]) -> int:
        ...
        design_count = sum(
            1 for keyword in system_design_keywords if keyword in text_lower
        )
    ...
    def _identify_strengths(
        ...
        if any(
            word in text_lower
            for word in ["architecture", "system design", "scalability"]
        ):
        ...
        if "skill_depth" in skill_analysis:
            advanced_skills = [
                s for s, d in skill_analysis["skill_depth"].items() if d == "advanced"
            ]
            if len(advanced_skills) >= 3:
                strengths.append(
                    f"Advanced proficiency in {len(advanced_skills)} core technologies"
                )
        ...
        modern_count = sum(
            1
            for skill in ["react", "typescript", "docker", "kubernetes", "aws"]
    ...
    def _identify_technical_concerns(
        self, resume_text: str, skill_analysis: Dict[str, Any]
    ) -> List[str]:
        ...
        if "skill_depth" in skill_analysis:
            ...
            for skill in matched_skills[:5]:  # Check top 5 skills
                if skill in skill_depth and skill_depth[skill] == "beginner":
                    concerns.append(
                        f"Claims '{skill}' but no evidence of depth - needs substantiation"
                    )
                    break
        ...
        # No system design
        if not any(
            word in text_lower for word in ["architecture", "design", "scalability"]
        ):
        ...
        if text_lower.count("various") > 2 or text_lower.count("multiple") > 3:
            concerns.append(
                "Vague technical descriptions - need specific implementation details"
            )
        ...
        missing_modern = sum(
            1
            for practice in modern_practices
    ...
    def _generate_comment(
        self, verdict: str, strengths: List[str], concerns: List[str]
    ) -> str:
        ...
        if verdict == "STRONG_YES":
            ...
            comment += (
                "Would be confident putting them on complex systems work immediately."
            )
            ...
            comment += (
                "Would need significant mentoring. Better suited for junior role."
            )
