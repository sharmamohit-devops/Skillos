...
class HRAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.soft_skill_keywords = {
            'leadership': ['led', 'managed', 'mentored', 'coached', 'directed', 'supervised'],
            'collaboration': ['collaborated', 'partnered', 'coordinated', 'team', 'cross-functional'],
            'communication': ['presented', 'communicated', 'documented', 'reported', 'facilitated'],
            'problem_solving': ['solved', 'resolved', 'improved', 'optimized', 'streamlined'],
            'initiative': ['initiated', 'launched', 'pioneered', 'established', 'founded']
        }
    ...
    def _calculate_culture_fit(self, resume_text: str) -> int:
        ...
        # Positive indicators
        positive_indicators = [
            'passion', 'enthusiastic', 'motivated', 'driven', 'dedicated',
            'innovative', 'creative', 'proactive', 'adaptable', 'flexible'
        ]
        ...
        # Professional tone
        if any(word in text_lower for word in ['professional', 'excellence', 'quality']):
            score += 5
    ...
    def _calculate_progression(self, resume_text: str, profile: Dict[str, Any]) -> int:
        ...
        responsibility_keywords = ['managed', 'led', 'directed', 'oversaw', 'supervised']
        responsibility_count = sum(text_lower.count(word) for word in responsibility_keywords)
    ...
    def _identify_red_flags(self, resume_text: str, profile: Dict[str, Any]) -> List[str]:
        ...
        if len(years) > 2:
            ...
            for i in range(len(years_int) - 1):
                if years_int[i+1] - years_int[i] > 2:
                    flags.append(f"Potential employment gap around {years_int[i]}-{years_int[i+1]}")
    ...
    def _identify_green_flags(self, resume_text: str, skill_analysis: Dict[str, Any]) -> List[str]:
        """Identify positive signals"""
        flags = []
        text_lower = resume_text.lower()
        
        # Strong leadership signals
        leadership_count = sum(text_lower.count(word) for word in ['led', 'managed', 'mentored'])
        ...
        if achievement_count >= 5:
            flags.append(f"Strong results orientation - {achievement_count} quantified achievements")
        ...
        if any(word in text_lower for word in ['certified', 'certification', 'licensed']):
            flags.append("Professional certifications present")
        
        return flags[:4]  # Top 4 green flags
    
    def _determine_verdict(self, culture_fit: int, progression: int, 
                          soft_skills: int, red_flags: List[str]) -> str:
    ...
    def _generate_comment(self, verdict: str, red_flags: List[str], 
        ...
        if verdict == "SHORTLIST":
            ...
            comment += "Would recommend phone screen to clarify concerns before proceeding."
