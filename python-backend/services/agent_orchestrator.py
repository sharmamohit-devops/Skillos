...
class AgentOrchestrator:
    ...
    async def run_simulation(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Simulation complete - Overall score: {overall_metrics['overall_score']}")
        
        return {
            "agent_reports": {
                report["agent"]: report for report in agent_reports
            },
            **overall_metrics
        }
    
    async def _run_agent_with_delay(self, agent, data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _calculate_overall_metrics(self, agent_reports: list) -> Dict[str, Any]:
        ...
        positive_verdicts = [
            'PASS', 'SHORTLIST', 'HIRE', 'YES', 'STRONG_YES'
        ]
        
        positive_count = sum(
            1 for report in agent_reports 
            if report['verdict'] in positive_verdicts
        )
        ...
        panel_consensus = f"{positive_count} of {len(agent_reports)} agents recommend advancing"
